...
- [x] **8.4 Machine-readability audit (bonus, not in the original plan — prompted by exam Q&A prep).** The 8.1–8.3 write-up was honest about *what* was done but hadn't been checked against whether it was machine-actionable, not just human-readable. Six pointed questions exposed real gaps: a stale identifier in a metadata record, no machine-readable metadata at all (Markdown only), no qualified/typed links between resources, no URL-encoded (`rightsUri`) licenses, no standard-schema provenance (Markdown + PNG only), and one figure whose generating script had been deleted. *Done:* added `docs/metadata/*.json` (DataCite 4.4 JSON, 4 records including a new software one), `docs/provenance.json` (W3C PROV-JSON), URL-encoded licenses, recommitted `notebooks/build_fig07_provenance_diagram.py`; full list in `docs/FAIR_and_provenance.md`'s "Machine-readability audit" section. *Watch out avoided:* didn't claim the fixes closed every gap — the homemade vocabulary and the sandbox-only DOI's lack of indexing are named as still-real, not papered over.
